...
int getMerge(const int pNum){
	...
	for (int i = 0; i < pNum; ++i){
		...
		do{
			std::istringstream iss(line);
			iss>>readId>>pSign>>pairId>>bitFg>>refId>>readPos>>rQual>>cgV1>>cgC1>>cgV2>>cgC2>>cgV3>>cgC3;
			...
			if(bitFg!=4){
				...
				if(mVal>mVisit[2*readId+pairId-1]){
					rVisit[2*readId+pairId-1]=(uint8_t)(i+1);
					qVisit[2*readId+pairId-1]=rQual;
					mVisit[2*readId+pairId-1]=mVal;
					s1Visit[2*readId+pairId-1]=s1Val;
	...
	for (unsigned i=0; i<readCount; ++i){
		if(rVisit[i]){
			while(getline(samFiles[rVisit[i]-1],line)){
				std::istringstream iss(line);
				iss>>readId>>pSign>>pairId>>bitFg>>refId>>readPos>>rQual>>cigStr>>rNext>>pNext>>tLen>>seqStr>>qualStr;
				if(2*readId+pairId-1==i){
					//new change to import Qual=0 to repetitive aln
					if(qVisit[i]!=0)
						comFile<<line<<"\n";
					else
						comFile<<readId<<pSign<<pairId<<"\t"<<bitFg<<"\t"<<refId<<"\t"<<readPos<<"\t"
