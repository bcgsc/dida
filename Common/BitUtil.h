...
/** Return the Hamming weight of the given bit array. */
static inline uint64_t popcount(char* data, size_t bits)
{
	uint64_t count = 0;
	size_t bytes = (bits + 7) / 8;
	size_t numInts = bytes / sizeof(uint64_t);
	size_t leftOverBytes = bytes % sizeof(uint64_t);
	uint64_t* intPtr = reinterpret_cast<uint64_t*>(data);
	for (size_t i = 0; i < numInts; i++) {
		count += ::popcount(intPtr[i]);
	}
	uint64_t lastInt = 0;
	char* bytePtr = reinterpret_cast<char *>(&lastInt);
	for (size_t i = bytes - leftOverBytes; i < leftOverBytes; ++i)
		bytePtr[i] |= data[i];
	count += ::popcount(lastInt);
	return count;
}
