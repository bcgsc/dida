...
std::vector< std::vector<int> > getAdj(const char *aName, std::vector<int> &lenArr, long &totLen){
	std::string line;
	std::ifstream adjFile(aName);
	int maxLen;
	while(getline(adjFile, line)){
...
void distPar(const char *aName, const int pNum, std::vector<int> &disArr){
	...
	for (int i = 0; i < pNum; ++i){
		...
		astm << bName << "-" << i+1 << "." << eName;
		ustm << bName << "-" << i+1 << ".fa";
		aPath = astm.str(); uPath = ustm.str();
...
int getPartition(const char *uName, const int pNum){
	clock_t sTime = clock();
